...
const STORAGE_KEY = "day_conductor_latest_v2";
...
function loadState() {
  try {
    const raw = localStorage.getItem(STORAGE_KEY) || localStorage.getItem("day_conductor_latest_v1");
...
function buildLifeBlocksForDate(dateS, lifeRaw) {
  ...
  if (hasClub && life.clubStart && life.clubEnd) {
    ...
    if (!Number.isFinite(info.baseEndMs)) info.baseEndMs = en;
    else info.baseEndMs = Math.max(info.baseEndMs, en);

    if (!Number.isFinite(info.middayAnchorMs) || en < info.middayAnchorMs) {
      info.middayAnchorMs = en;
    }
  }
...
function earliestStudyStartMs(dateS, occBlocks, lifeRaw) {
  ...
  const lessonEnds = (occBlocks || []).filter(b => b.name === "授業").map(b => b.endMs);
  if (lessonEnds.length) earliest = Math.max(earliest, Math.max(...lessonEnds));

  const clubEnds = (occBlocks || []).filter(b => b.name === "部活").map(b => b.endMs);
...
function openRunner(taskId, segmentEndMs = null) {
  ...
  function renderRunner() {
    ...
    if (segmentEndMs && Number.isFinite(segmentEndMs)) {
      ...
      timeSmall.textContent = "（NOW→この区間の終了）";
    } else {
...
function renderStudy() {
  ...
  if (!list.length) {
    ...
    list.forEach((t, idx)=>{
      ...
      const steps = getTaskSteps(t);
      const tag = hasRealSteps(t) ? `範囲 ${steps.length} / 見積 ${estMin}分` : `見積 ${estMin}分`;
    ...
    if (!dayBlocks.length) {
      ...
      dayBlocks.forEach(b=>{
        ...
        card.addEventListener("click", ()=>{
          if (b.kind === "study") openRunner(b.meta.taskId, b.endMs);
          else {
            const bd = el("div","grid1");
            bd.appendChild(el("div","", b.name));
            bd.appendChild(el("div","note", fmtRange(b.startMs, b.endMs)));
